...
Use "Partial stats reload" to request only `stats`. Deferred stats and activity load after the first render. Navigation is a once prop with a ten-minute expiry; "Force navigation refresh" explicitly requests a fresh value. Scrolling loads three user pages through manual infinite-scroll metadata.

Submit a one-character name on the Create user page. The server stores validation errors in the private flash cookie, redirects with 303, and returns them under the `createUser` error bag. A valid name redirects to Users and displays page flash once.
