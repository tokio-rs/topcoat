...
Thanks for your interest in Topcoat. This guide explains, in short, how a change gets from your machine into the repository.
...
A stable toolchain is enough to build and test everything, and `rust-toolchain.toml` pins stable. You also need a nightly toolchain for the formatter and the doc check:
...
The examples are the fastest way to try a change against a real app. Each one is a workspace member, so running `topcoat dev` inside `examples/hello-world` (or any other example directory) builds and serves it. An example covers one feature. A demo in `demos/` is a whole application that uses many features together, so it is the better place to see how a change works in context.
...
Two conventions are worth knowing up front, because they affect almost every diff:

- Use plain ASCII everywhere: in code, docs, and commit messages. Write `->` instead of an arrow character and `...` instead of an ellipsis, and do not use em dashes.
