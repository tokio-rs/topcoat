//! Micro-benchmarks for [`ViewHandle::render`].
//!
//! Each scenario builds a [`View`] once, up front, and then times only the
//! render pass that turns the resolved [`ViewHandle`] into an HTML `String`.
//! Building the view (running components, evaluating `format!`
//! interpolations, and pushing view parts) happens in setup. The measured
//! work is writing static markup, escaping dynamic text, and formatting
//! numbers into the output.
//!
//! The first scenarios each isolate one cost: static markup, text escaping,
//! numeric formatting, and attribute output. The last one is a realistic,
//! component-based product grid at several sizes, to show how rendering
...
/// A dense table of integers. Every cell renders a number, isolating numeric
/// formatting from markup.
...
/// The path of the heroicons solid star icon.
...
///
/// Like the scenarios above, it passes the request context to `view!` with the
/// `cx =>` form. It also invokes a component (`product_card`) for each card.
