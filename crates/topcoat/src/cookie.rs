...
//! ## Reading and writing
//!
//! `cookies(cx)` returns the request's root jar. The incoming `Cookie` header is
//! parsed on first access and memoized for the rest of the request, so repeated
//! calls are cheap and see the same pending changes. Bring the [`Cookies`] trait
//! into scope for the `get`, `add`, and `remove` methods.
//!
//! A cookie is a [`Cookie`] from the `cookie` crate. Build a bare one with
//! `Cookie::new`, or use `Cookie::build` for attributes:
//!
//! ```rust,ignore
//! use topcoat::{
//!     Result,
...
//! ```
//!
//! `add` and `remove` accept anything that implements `Into<Cookie>`, so a plain
//! `(name, value)` tuple works too when you don't need attributes:
//!
//! ```rust,ignore
//! jar.add(("theme", "dark"));
//! ```
...
//!
//! For cookies with several attributes, the [`cookie!`] macro is more compact
//! than the builder. It mirrors the `Set-Cookie` header: the `name = value` pair
//! first, then any number of `;`-separated attributes.
//!
//! ```rust,ignore
...
//!
//! Rather than repeat the same attributes on every cookie, layer them onto the
//! jar with the [`Cookies`] combinators. Each one wraps the jar and applies an
...
//! ```
//!
//! The same pairs exist for `path`, `domain`, and `max_age`. Because the
//! combinators consume and return the jar, build the configured jar once and
//! reuse it for several writes.
//!
//! Rather than repeat that chain at every call site, the idiomatic pattern is to
//! write your own `cookies` helper that bakes in your app's defaults and have
//! the rest of your code use it instead of the one from `topcoat::cookie`:
//!
//! ```rust,ignore
//! use topcoat::{
//!     context::Cx,
...
//! ```
//!
//! Every handler that calls this `cookies(cx)` gets the defaults for free, and
//! you can tighten them in one place. The same approach works for signed or
//! private jars — return `impl Cookies` and layer on whatever combinators your
//! app needs.
//!
//! For anything the named combinators don't cover, [`map`](Cookies::map) is the
//! escape hatch — it runs a closure on every added cookie:
//!
//! ```rust,ignore
...
//! ## Name prefixes
//!
//! [RFC 6265bis] cookie name prefixes ask the browser to enforce extra
//! constraints based on the cookie's name. Topcoat applies the prefix *and* its
...
//! ```
//!
//! As with attributes, each prefix has a `default_*` form that fills the
//! required attributes only when unset, and an `override_*` form that forces
//! them for guaranteed RFC compliance. Use `override_*` unless you have a reason
//! to let a caller's value stand.
//!
//! [RFC 6265bis]: https://datatracker.ietf.org/doc/html/draft-ietf-httpbis-rfc6265bis#name-cookie-name-prefixes
//!
//! ## Signed cookies
//!
//! A **signed** cookie is tamper-proof but still readable by the client —
//! useful when the value isn't secret but must not be forged (a user id, a
//! feature flag). Signing wraps the jar with a [`Key`]; reads return `None` when
//! a cookie is missing or its signature doesn't verify.
//!
//! ```rust,ignore
...
//! ## Private cookies
//!
//! A **private** cookie is encrypted with AES-256-GCM, so its value is both
//! tamper-proof *and* unreadable by the client. Use it for anything sensitive.
//! The cookie's name is bound into the ciphertext, so the name must match on
//! write and read — which it does automatically, however you compose this layer.
//!
//! ```rust,ignore
//! use topcoat::cookie::{Cookies, Key, cookie, cookies};
//!
...
//! ```
//!
//! Signing and private encryption operate on the cookie value (and, for private,
...
//! ```
//!
//! Then `signed_cookies(cx)` and `private_cookies(cx)` give you a wrapped jar
//! using that registered key, with no plumbing:
//!
//! ```rust,ignore
//! use topcoat::{
//!     Result,
