...
/// Navigation showing the path to the current page.
///
/// Place items inside a `breadcrumb_list`. Use links for earlier pages and a
/// `breadcrumb_page` for the current one. Attributes are forwarded to the
/// `<nav>`, and classes are appended.
///
/// ```ignore
/// view! {
///     breadcrumb(
...
/// A link to an ancestor of the current page.
///
/// Pass its destination through the `href` attribute.
#[component]
...
/// The current page, marked with `aria-current="page"`.
#[component]
...
/// A decorative chevron between breadcrumb items.
#[component]
...
/// An ellipsis representing omitted breadcrumb items.
///
