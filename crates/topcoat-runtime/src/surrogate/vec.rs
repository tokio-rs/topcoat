...
use crate::{
    SliceSurrogate, Surrogate, Surrogated, impl_surrogate, impl_surrogate_mut, impl_surrogate_ref,
};
    ...
    T::Surrogate: Deserialize<'de>,
{
        ...
        D: serde::Deserializer<'de>,
    {
        ...
        if value.bits != usize::BITS {
            return Err(de::Error::custom("collection pointer width does not match target"));
        }
        Ok(Self(value.v.into_iter().map(Surrogate::into_real).collect()))
    }
}
...
mod tests {
    ...
    fn serializes_owned_and_borrowed_sequences_through_element_surrogates() {
        ...
        assert_eq!(serde_json::to_value((&values).into_surrogate()).unwrap(), wire);
    ...
    fn round_trips_empty_and_nested_vectors() {
        for values in [vec![], vec![vec![], vec![Some(42u64), None, Some(u64::MAX)]]] {
    ...
    fn methods_borrow_elements_and_clone_owned_storage() {
        ...
        assert!(surrogate.get(usize::MAX.into_surrogate()).into_real().is_none());
