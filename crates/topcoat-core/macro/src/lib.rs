...
/// The `#[memoize]` attribute caches the result of a function for the duration of a single request,
/// keyed by its arguments. Call the same function twice with the same arguments inside one request
/// and the body runs only once — the second call returns the cached value.
///
/// This is the per-request equivalent of memoization in libraries like React's `cache`: it's not a
/// global cache and it's not persisted across requests. Each new request starts with an empty
/// cache.
///
/// ## Setup
///
...
/// ```
///
/// That's it. Calling `get_user(cx, 42).await` from anywhere in the request — a page, a layout, a
/// component — runs the body the first time and returns the cached `User` for every subsequent call
/// with `id == 42`. The function's return type `T` is rewritten to `&T` that has the same lifetime
/// as `&cx`.
///
/// Top-level `Option<T>` and `Result<T, E>` return types are borrowed ergonomically: the macro
/// calls `.as_ref()` on the cached value and returns `Option<&T>` or `Result<&T, &E>` instead of
...
/// ```
///
/// For async functions, concurrent callers with the same arguments share a single in-flight future.
/// If two parts of your page render in parallel and both call `fetch_post(cx, "hello")`, the
/// database is queried once and both callers await the same result.
///
/// ## What gets cached
///
...
/// ```
///
/// Each `#[memoize]` function has its own independent cache slot, so two functions with the same
/// argument types don't collide.
///
...
/// - For a borrowed argument of type `&P`: `P: ToOwned` with `P::Owned: Hash + Eq + Send + Sync +
///   'static`.
/// - The return type `T` must be `Send + Sync + 'static`.
///
/// Most everyday types — `i32`, `String`, `&str`, `Uuid`, your own `#[derive(Hash, Eq, PartialEq,
/// Clone)]` structs — satisfy these out of the box.
///
/// ## When to reach for it
///
/// Use `#[memoize]` when the same data may be requested multiple times during a single request and
/// recomputing it is wasteful. Common cases:
///
/// - **Database lookups** that several components need (current user, settings, feature flags).
/// - **Deduplication of fan-out fetches** when components render in parallel and would otherwise
///   hit the same endpoint repeatedly.
///
/// It is *not* a substitute for a long-lived cache (Redis, an LRU, etc.). Cross-request caching is
/// a separate concern and should be layered behind your data access functions.
///
...
/// ```
///
/// The layout and the page each call `current_user(cx)`, but the database is queried at most once
/// per request.
