...
/// `https://example.com`.
///
/// Use it to build absolute URLs. It must be an `http` or `https` URL with
/// a host and no query or fragment. An optional path prefix supports apps
/// mounted below the host root, such as `https://example.com/app`.
///
