...
mod tests {
    ...
    fn html_derives_its_text_alternative_by_default() {
        let mail = base()
            .html(Markup::unescaped_unchecked("<h1>Hi</h1><p>Bye now</p>"))
            .build();
        let wire = formatted(mail);
    ...
    fn opted_out_text_leaves_a_single_html_part() {
        let mail = base()
            .html(Markup::unescaped_unchecked("<h1>Hi</h1>"))
            .text(TextBody::None)
            .build();
        let wire = formatted(mail);
    ...
    fn empty_derived_text_is_not_sent() {
        let mail = base()
            .html(Markup::unescaped_unchecked("<img src=\"x.png\">"))
            .build();
        let wire = formatted(mail);
    ...
    fn inline_attachments_relate_to_the_html_body() {
        let mail = base()
            .text("Hi Bob")
            .html(Markup::unescaped_unchecked("<img src=\"cid:logo\">"))
