...
A `Param` or `CatchAll` segment without a `rename` is named after the module, as-is. Declaring a [`path_param!`](macro.path_param.html) in a module emits the matching segment override automatically, so do not also call `segment!` in that module. A manual `Param` or `CatchAll` declaration creates a captured segment but no typed accessor; read it from [`RawPathParams`](struct.RawPathParams.html) in the request context.
...
`RawPathParams` returns its encoded tail. Use `path_param!(*name)` to iterate over separately decoded segments.
