...
A layer at the root path wraps every request, matched or not: a request that matches no handler still runs the root layers, and its 404 (path matched nothing) or 405 (path matched, method did not) comes back through them as the `Err` returned by `next.run`. Layers at any other path wrap only the handlers they match, so a 404 or 405 never runs them.
