...
///
/// Call [`next_field`](Multipart::next_field) to read each [`Field`] and its
/// metadata. Read a field all at once with [`bytes`](Field::bytes), or consume
/// its chunks through [`Stream`]. Finish or drop a field before reading the
/// next one.
///
