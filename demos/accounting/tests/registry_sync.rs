...
fn theme_matches_registry() {
    ...
    let hint = "re-install the theme by deleting it along with components.toml \
        and running `topcoat ui init --theme neutral` in demos/accounting";
    assert!(
...
fn components_match_registry() {
    ...
    for name in registry.names() {
        ...
        let hint = format!("run `topcoat ui add {name} --overwrite` in demos/accounting");
        assert!(
    ...
    for entry in entries {
        ...
        assert!(
            ...
            "{} is not a component of the built-in registry; \
             remove it with `topcoat ui remove` in demos/accounting, or move it out of {}",
